...
import qualified Data.Vector.SEXP.Mutable  as VSM
...
import Language.R.Instance (R, runRegion, unsafeRunRegion)
...
foreign export ccall sliceR2 :: Ptr (SEXP V 'R.Real) -> Ptr CInt -> Ptr CInt -> Ptr (SEXP V 'R.Real) -> IO ()
sliceR2 :: Ptr (SEXP V 'R.Real) -> Ptr CInt -> Ptr CInt -> Ptr (SEXP V 'R.Real) -> IO ()
sliceR2 vectorR i n result = do
  vectorR <- peek vectorR
  i <- peek i
  n <- peek n
  let vector = VSM.fromSEXP vectorR
  let slicedVector = VSM.slice (fromIntegral i) (fromIntegral n) vector
  let slicedVectorR = release <$> VSM.toSEXP (VSM.release slicedVector) :: R V (SEXP V 'R.Real)
  (>>=) (unsafeRunRegion slicedVectorR) (poke result) 
